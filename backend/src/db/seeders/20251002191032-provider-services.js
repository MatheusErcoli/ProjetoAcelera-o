...
module.exports = {
  async up(queryInterface) {
    ...
    if (users.length === 0 || services.length === 0) {
      console.log(
        "Usuário João ou serviços não encontrados, pulando criação de provider_services"
      );
      return;
    }
    ...
    const existingCombinations = existingProviderServices.map(
      (ps) => `${ps.provider_id}-${ps.service_id}`
    );
    ...
    const providerServicesToInsert = [
      ...
      (ps) =>
        !existingCombinations.includes(`${ps.provider_id}-${ps.service_id}`)
    );

    if (providerServicesToInsert.length > 0) {
      await queryInterface.bulkInsert(
        "provider_services",
        providerServicesToInsert
      );
    }
  },
